...
* Ever seen code like this?
      ...
      <%= return link_to('Edit', edit_something_path(something), :class => 'edit round', :title => "Edit record for #{record.owner}") %>
...
The downside is that the implementation still needs to be as
described in - module SomethingHelper
